...
class api {
	static $file = 'api.php';
	static $base = 'api';
	static $json = ['out' => '0', 'msg' => '', 'data' => '', 'dict' => '', 'code' => '', 'line' => ''];
	static $cors = FALSE;
	static $acch = '';
		...
		if (!empty($_SERVER['HTTP_ORIGIN'])) {
			if (self::$cors) {
				...
				if ($allow) {
					...
					header('Access-Control-Allow-Headers: Content-Type,Token,Client,'.self::$acch);
					header('Access-Control-Expose-Headers: Content-Type,Token,Client,'.self::$acch);
