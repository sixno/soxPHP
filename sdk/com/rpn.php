...
class rpn {
	...
	const PATTERN_EXP    = '/((?:[\x{4e00}-\x{9fa5}a-zA-Z0-9_]+)|(?:[\(\)\+\-\*\/])){1}/u';
	...
	*/
	static function calculate($exp, $exp_values) {
	...
	//将中缀表达式转为后缀表达式
	static function nifix2rpn($input_queue) {
		$exp_stack = [];
		$output_queue = [];
	...
	static function calculate_value($output_queue, $exp_values) {
		$res_stack = [];
