...
class validation {
	...
	static function greater_to($field, $field2) {
		...
		if (is_numeric($field2)) {
			...
			if (!isset(self::$input[$field])) return TRUE;

			$number = (string)self::$input[$field];
		}

		if (!isset(self::$input[$field]) || $number === '') return TRUE;

		$result = self::$input[$field] >= $number;

		if (!$result) {
			self::$error[$field] = sprintf(self::$err_t['greater_to'],self::$label[$field], $number);
		}
	...
	static function less_to($field, $field2) {
		...
		if (!$result) {
			self::$error[$field] = sprintf(self::$err_t['less_to'],self::$label[$field], $number);
		}
