...
class img
{
	static $img_library = 'imagemagick';
	static $img_libpath = '';

	static $imglib = NULL;

	static function autocrop($image,$new_width,$new_height,$quality = 90,$density = 72)
	{
		if(!is_array($image))
		{
			$source_image = $image;
		...
		$conf['image_library']  = self::$img_library;
		$conf['library_path']   = self::$img_libpath;
		$conf['source_image']   = $source_image;
		$conf['new_image']      = $new_image;
		$conf['quality']        = $quality;
		$conf['density']        = $density;
		$conf['maintain_ratio'] = TRUE;

		$imginfo = getimagesize($conf['source_image']);
		$width   = $imginfo[0];
		$height  = $imginfo[1];
		...
		if($ratio > $new_ratio)
		{
			if($height < $new_height) $conf['width'] = $new_height*$ratio;
			$conf['height'] = $new_height;
			$conf['master_dim'] = 'height';

			self::$imglib->initialize($conf);
			self::$imglib->resize();
			self::$imglib->clear();
			unset($conf['master_dim']);
		}

		if($ratio < $new_ratio)
		{
			if($width < $new_width) $conf['height'] = $new_width/$ratio;
		...
		if($ratio == $new_ratio)
		{
			if($width < $new_width) $conf['height'] = $new_height;
			$conf['width'] = $new_width;
		...
		{
			$conf['image_library']  = self::$img_library;
			$conf['source_image']   = $new_image;
			$conf['maintain_ratio'] = FALSE;

			$imginfo = getimagesize($new_image);
			$width   = $imginfo[0];
			$height  = $imginfo[1];
			...
			if($height == $new_height)
			{
				...
				$x_axis = ($width-$use_width)/2;
				$y_axis = 0;

				$conf['width']  = $use_width;
				$conf['height'] = $use_height;
				$conf['x_axis'] = $x_axis;
				$conf['y_axis'] = $y_axis;
			...
			{
				$use_width  = $width;
				$use_height = $new_height;

				$x_axis = 0;
				$y_axis = ($height-$use_height)/2;
	...
	static function resize($source_image,$new_image,$maintain_ratio = TRUE,$quality = 90,$create_thumb = TRUE)
	{
		$conf['image_library']  = self::$img_library;
		$conf['library_path']   = self::$img_libpath;
		$conf['source_image']   = $source_image;
		$conf['quality']        = $quality;
		$conf['create_thumb']   = $create_thumb;
		$conf['thumb_marker']   = '';
		$conf['maintain_ratio'] = $maintain_ratio;
		...
		foreach($new_image as $key => $value)
		{
			...
			{
				self::resize($source_image,$value,$maintain_ratio,$quality,$create_thumb);
			}
		}

		if(isset($conf['master_dim']))
		{
			$resize_go = TRUE;

			if(isset($conf['new_image']))
			{
				$dir_path = substr($conf['new_image'],0,strrpos($conf['new_image'],'/')+1);
			...
			if($conf['master_dim'] == 'crop')
			{
				$resize_go = FALSE;
				self::autocrop(array($conf['source_image'],$conf['new_image']),$conf['width'],$conf['height'],$quality);
			}
	...
	static function crop($image,$x_axis,$y_axis,$width,$height = NULL,$quality = 90,$density = 72)
	{
		if(empty($height)) $height = $width;

		$conf['image_library']  = self::$img_library;
		$conf['library_path']   = self::$img_libpath;
		$conf['source_image']   = $image;
		$conf['quality']        = $quality;
		$conf['density']        = $density;
		$conf['x_axis']         = $x_axis;
		$conf['y_axis']         = $y_axis;
...
class imglib
{
	...
	var $x_axis				= '';
	var	$y_axis				= '';
	var $density            = 72;       // when iamgemagick is used, it can be work.
